...
public:
  ...
  // matrix_expr's at() calls derived at()
...
private:
  ...
  // using 1D vector gives contiguous memory, but maybe excess overhead from vector
  std::vector<T> matrix_;
  
public:  
  // could use a variety of different constructors
  // from 1D or 2D containers, etc.

  matrix<T>() = default;
  
  // ctor from 1d vector, would be more useful for testing with
  // generated stuff
...
public:
  matrix_sum(E1 const& u, E2 const& v) : lhs_(u), rhs_(v) {
    assert((lhs_.num_rows() == rhs_.num_rows()) && (lhs_.num_cols() == rhs_.num_cols()));
...
template<typename E1, typename E2> // specialize, lhs is scalar
...
template<typename E1, typename E2> // specialize, rhs is scalar
